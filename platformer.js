(function() { // module pattern
  ...
  var killaudio = new Audio('kill.mp3');
  ...
  var MAP      = { tw: 64, th: 48 },
      ...
      COLOR    = { BLACK: '#000000', YELLOW: '#141e58', BRICK: '#D95B43', PINK: '#9e630e', PURPLE: '#886565', GREY: '#333', SLATE: '#53777A', GOLD: 'gold' },
  ...
  function updateMonster(monster, dt) {
    if (!monster.dead) {
      ...
      if (overlap(player.x, player.y, TILE, TILE, monster.x, monster.y, TILE, TILE)) {
		...
		{
			killMonster(monster);killaudio.play();
		}
        else
		{
          killPlayer(player);dieaudio.play();
		}
      }
    }
  }
  ...
  function collectTreasure(t) {
    player.collected++;
    t.collected = true;coinaudio.play();
  }
  ...
  function updateEntity(entity, dt) {
    ...
    if (entity.jump && !entity.jumping && !falling) {
      ...
      entity.jumping = true; jumpaudio.play();
    }
  ...
  function renderPlayer(ctx, dt) {
    ...
    ctx.fillStyle = '#3aee1d';
    ctx.fillRect(player.x + (player.dx * dt), player.y + (player.dy * dt), TILE, TILE);
	ctx.strokeStyle = '#ff0000';
	ctx.strokeRect(player.x + (player.dx * dt), player.y + (player.dy * dt), TILE, TILE);
    var n, max;
...
/*
  ...
  */
  ...
  function setupEntity(obj) {
	...
	{
		if(obj.properties[0].name != undefined && obj.properties[0].name=="maxdx")
		{
			tempmaxdx = obj.properties[0].value;
		}
		if(obj.properties[1].name != undefined && obj.properties[1].name=="maxdx")
		{
			tempmaxdx     = obj.properties[1].value;
		}
	}
	var tempmaxdy;
	if("properties" in obj)
	{
		if(obj.properties[0].name != undefined && obj.properties[0].name=="maxdy")
		{
			tempmaxdy = obj.properties[0].value;
		}
		if(obj.properties[1].name != undefined && obj.properties[1].name=="maxdy")
		{
			tempmaxdy     = obj.properties[1].value;
		}
	}
    entity.maxdx    = METER * (tempmaxdx || MAXDX);
    entity.maxdy    = METER * (tempmaxdy   || MAXDY);
	...
	if("properties" in obj)
	{
		if(obj.properties[0].name != undefined && obj.properties[0].name=="left")
		{
			entity.left = obj.properties[0].value;
		}
		if(obj.properties[1].name != undefined && obj.properties[1].name=="left")
		{
			entity.left     = obj.properties[1].value;
		}
		if(obj.properties[0].name != undefined && obj.properties[0].name=="right")
		{
			entity.right = obj.properties[0].value;
		}
		if(obj.properties[1].name != undefined && obj.properties[1].name=="right")
		{
			entity.right     = obj.properties[1].value;
		}
	}
  ...
  document.addEventListener('keyup',   function(ev) { return onkey(ev, ev.keyCode, false); }, false);
	...
	$.getJSON( "level.json", function( json ) {
